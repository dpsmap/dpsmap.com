...
$verifyEmail = function ($email, $OTP) use ($link) {
    $query = "SELECT * FROM `otp` WHERE `customer_email` = '{$email}'";
    $queryResult = mysqli_query($link, $query);
    if (mysqli_num_rows($queryResult) > 0) {
        $row = mysqli_fetch_assoc($queryResult);
        if ($row['otp_code'] === $OTP) {
            $query = "UPDATE `otp` SET `email_status` = 'verified' WHERE `customer_email` = '{$email}'";
...
if (isset($_POST['verify_btn'])) {
    ...
    $email = $sanatization($_POST['email']);
    $OTP = $sanatization($_POST['otp']);
    $username = $_SESSION['username'];
    $mapFile =  $_SESSION['map_to_download'];
    $currentCustomerNumber = $_SESSION['customer_number'];


    $user_activation_hash = sha1(uniqid(mt_rand(), true));

    if ($verifyEmail($email, $OTP) === true) {
        ...
        if (!$mail->Send()) {
            ...
            $query = "UPDATE `customers` SET `merge_status` = 'mail sent', `email_status` = 'verified'  WHERE `email`= '{$email}' AND `customer_no` = {$currentCustomerNumber}";
            mysqli_query($link, $query);
            $query = "INSERT INTO `token` (`activation_code`, `customer`) VALUES ('{$token}', {$currentCustomerNumber})";
            mysqli_query($link, $query);
            session_destroy();
            header("Location:../download-sent/?email={$email}");
        }
    } else {
        $logs[] = $verifyEmail($email, $OTP);
    }
}
